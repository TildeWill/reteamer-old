...
RSpec.describe People do
  it "let's you save multiple objects for the same day" do
    ...
    expect(People.find_for(6.days.from_now.to_date).count).to eq(3)
    expect(People.find_for(2.days.from_now.to_date).count).to eq(2)
  ...
  it "let's you update the same object" do
    ...
    person.update(2.days.from_now.to_date, first_name: "Paul")
    person.update(2.days.from_now.to_date, first_name: "Paulz")
    person.update(1.days.from_now.to_date, first_name: "Mary")
    ...
    mary = People.find_for(1.days.from_now.to_date).first
    expect(mary.first_name).to eq("Mary")
    expect(mary.last_name).to eq("Singer")
    paulz = People.find_for(2.days.from_now.to_date).first
